 ...
 *        QtCommand to the sender's undo stack, as in processChange, below.
 ...
 *      > Lastly, if !--recursionDepth and !desc.isNull(), then a QtCommand should
...
public:
    ...
    /*
    void pushUndoCommand ( QtCommand* cmd );
    void setUndoStack ( QtUndoStack* s );
    */
...
signals:
    void changed ( QString key, QString value, QString desc );
    void reset ();
    void pushThisCommand ( QtCommand* cmd );
...
class GEVCommand : public QtCommand
{
    Q_OBJECT
