...
{
    undoStack = 0;//new QtUndoStack( this );
...
void GroupTable::loadGroups ( QStringList paths ) {
    ...
    for ( it = absFilePaths.begin() ; it != absFilePaths.end() ; ++it ) {
        ...
        if ( maxGroupOrder > -1 ) {
            ...
            if ( max ) {
                int order = QStringList::split( ' ', re.cap( 1 ) ).count();
...
void GroupTable::applyPreferences ( GEPreferences *p )
{
    ...
    // is called from a QtCommand that remembers the old ones for us:
    lastPrefs = p;
    applyPreferences();
}
...
void GroupTable::setUndoStack ( QtUndoStack* stack )
{
    ...
    : table( table )
{
    ...
    setDescription( "enter " + newvalue + " in cell" );
}
    ...
    colJustSorted( colJustSorted ), wayJustSorted( wayJustSorted )
{
    ...
    setDescription( QString( "%1 groups by %2" )
    ...
    draggedThisRow( draggedFrom ), firstRedo( TRUE )
{
    ...
    setDescription( QString( "move row %1 to row %2" )
    ...
    : table( table ), draggedThisColumn( draggedFrom ), firstRedo( TRUE )
{
    ...
    setDescription( "move column "
                    + table->horizontalHeader()->label( intoThisColumn ) );
}
    ...
    : table( table ), column( column ), hide( hide )
{
    ...
    setDescription( QString( "%1 column %2" )
                    .arg( hide ? "hide" : "show" ).arg( colname ) );
}
    ...
    : table( table ), column( column )
{
    ...
    setDescription( "delete column " + colname );
}
    ...
    : table( table ), column( column ), newname( text )
{
    ...
    setDescription( "rename column " + oldname + " to " + newname );
}
...
{
    setDescription( "create column " + colname );
}
    ...
    : table( table )
{
    ...
    *newPrefCopy = *newPrefs; // overwrite data
    setDescription( "change options/appearance" );
}
...
GTCmdResizeColumn::GTCmdResizeColumn (
    ...
    firstRedo( TRUE )
{
    setDescription( "resize column " + table->horizontalHeader()->label( column ) );
}
...
bool GTCmdResizeColumn::mergeMeWith ( QtCommand* c )
{
    if ( qstrcmp( c->className(), className() ) ) return FALSE;
