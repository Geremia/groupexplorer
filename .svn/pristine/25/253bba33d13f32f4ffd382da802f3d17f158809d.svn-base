...
GEShCmdInsertText::GEShCmdInsertText ( GESheetWindow* window )
    : window( window )
{
    setDescription( "insert new text box" );
...
GEShCmdInsertRect::GEShCmdInsertRect ( GESheetWindow* window )
    : window( window )
{
    setDescription( "insert new rectangle" );
    GESheet* sheet = window->sheet();
...
{
    setDescription( "insert new visualizer" );
...
{
    setDescription( "move sheet object(s)" );
}
...
{
    setDescription( "select sheet object(s)" );
...
{
    setDescription( "change text box contents" );
...
{
    setDescription( QString( "%1 font size" ).arg( increase ? "increase" : "decrease" ) );
}
    ...
    : sheetview( sheetview )
{
    ...
    setDescription( "cut sheet item(s)" );
...
GEShCmdPasteItems::GEShCmdPasteItems ( Q3CanvasItemList items )
    : items( items )
{
    setDescription( "paste sheet item(s)" );
}
    ...
    : firstTime( TRUE ), item( item )
{
    ...
    setDescription( "resize sheet item" );
}
...
GEShCmdPushSelectionZ::GEShCmdPushSelectionZ ( GESheetView* sheetview, bool backwards )
    : backwards( backwards )
{
    setDescription( backwards ? "push selection back" : "bring selection forward" );
    items = sheetview->selectedItems();
}
...
GEShCmdConnectWithLine::GEShCmdConnectWithLine ( QtUndoStack* undoStack )
    : GEShCmdParameterized( undoStack )
{
    setDescription( "insert connecting line" );
}
...
GEShCmdConnectWithMorphism::GEShCmdConnectWithMorphism (
        QtUndoStack* undoStack, QString morphName )
    : GEShCmdParameterized( undoStack ), tmp( morphName )
{
    setDescription( "insert homomorphism" );
}
      ...
      newmany( many )
{
    ...
    setDescription( "change morphism" );
}
