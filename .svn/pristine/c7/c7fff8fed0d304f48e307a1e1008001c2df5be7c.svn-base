...
/*
void GEVSmall::pushUndoCommand ( QUndoCommand* cmd )
{
    undoStack->push( cmd );
}

void GEVSmall::setUndoStack ( QUndoStack* s )
{
    undoStack = s;
}
*/
...
void GEVisualizer::reset ( QString desc )
{
    ...
    // QUndoCommand object, or from the constructor, and should therefore be obeyed:
    QStringList keys = orderedKeys();
    for ( int i = 0 ; i < keys.count() ; i++ )
...
void GEVisualizer::processChange ( QString key, QString value, QString desc )
{
    ...
    if ( oldvalue != value ) {
        ...
        // QUndoStack, either that of the small or large:
