...
LinkCommand* LinkCommands::getNewCommand ( QStringList call, QWidget* originator )
{
    ...
    qWarning( "Error: no such link command: %s %s",
              commandIdentifier.toLatin1().constData(),
              call.join( " " ).toLatin1().constData() );
    return 0; // bad
}
    ...
    : LinkCommand ( parameters, originator )
{
    ...
    if ( parameters.size() ) {
        ...
        if ( !newnotes.isNull() ) {
            setText( "change notes for " + GroupFile::decodePrimaryName( parameters[0] ) );
        }
    } else {
        ...
        if ( ok ) {
            ...
            if ( gf ) {
                if ( dialog.exec( gf->getPrimaryName(), gf->reps[0], oldrep ) ) {
                    ...
                    setText( "change representation for " + gf->getPrimaryName().text );
                }
            } else {
                qWarning( "Error: edit name scheme link: group not found: %s",
                          parameters[0].toLatin1().constData() );
            }
        } else {
            qWarning( "Error: edit name scheme link: "
                      "second parameter must be nonnegative integer (not %s)",
                      parameters[1].toLatin1().constData() );
        }
    } else {
        qWarning( "Error: edit name scheme link with fewer than 2 parameters: %s",
                  parameters.join( " " ).toLatin1().constData() );
    }
}
    ...
    if ( parameters.size() ) {
        ...
        if ( gf ) {
            if ( dialog.exec( gf->getPrimaryName(), gf->reps[0], blanks ) ) {
                newrep = dialog.getRepresentation();
                setText( "add new representation for " + gf->getPrimaryName().text );
            }
        } else {
            qWarning( "Error: new name scheme link: group not found: %s",
                      parameters[0].toLatin1().constData() );
        }
    } else {
    ...
    if ( parameters.size() > 1 ) {
        ...
        if ( ok ) {
            if ( MainWindow::original()->preferences->numGroupReps( parameters[0] )
                    > repNum ) {
                setText( "delete representation from "
                         + GroupFile::decodePrimaryName( parameters[0] ) );
            } else {
                qWarning( "Error: remove name scheme link: %s"
                          " has no naming scheme numbered %s",
                          parameters[0].toLatin1().constData(),
                          QString::number( repNum ).toLatin1().constData() );
            }
        } else {
            qWarning( "Error: edit name scheme link: second parameter must be nonnegative "
                      "integer (not %s)", parameters[1].toLatin1().constData() );
        }
    } else {
        qWarning( "Error: edit name scheme link with fewer than 2 parameters: %s",
                  parameters.join( " " ).toLatin1().constData() );
        ...
        if ( gf ) {
            ...
            if ( ok ) {
                ...
                if ( newRepNum < numUserDefinedReps + numLoadedReps ) {
                    setText( "change default representation for " +
                             GroupFile::decodePrimaryName( parameters[0] ) );
                } else {
                    qWarning( "Error: choose name scheme link with too large an index: %s",
                              parameters[1].toLatin1().constData() );
                }
            } else {
                qWarning( "Error: choose name scheme link: second parameter must be "
                          "nonnegative integer (not %s)",
                          parameters[1].toLatin1().constData() );
            }
        } else {
            qWarning( "Error: choose name scheme link with bad group code: %s",
                      parameters[0].toLatin1().constData() );
        }
    } else {
        qWarning( "Error: choose name scheme link command with fewer than 2 parameters: %s",
                  parameters.join( " " ).toLatin1().constData() );
    }
}
            ...
            if ( parameters.size() > 1 ) {
                ...
                if ( gf ) {
                    ...
                    qWarning( "showVisualizer link command: could not find group "
                              "with code \"%s\"", parameters[1].toLatin1().constData() );
            } else
                qWarning( "showVisualizer link command: group code missing: %s",
                          parameters.join( " " ).toLatin1().constData() );
        }
    }
}
...
void CreateEmbeddingSheetLC::redo ()
{
    ...
    if ( !G1 ) {
        qWarning( "Could not find %s in library", parameters[0].toLatin1().constData() );
        return;
    }
    GroupFile* G2 = GroupTable::findGroup( parameters[1] );
    if ( !G2 ) {
        qWarning( "Could not find %s in library", parameters[1].toLatin1().constData() );
        return;
    }
...
void ShowZnmIsomorphismLC::redo ()
{
    ...
    if ( ( m < 2 ) || ( n < 2 ) || ( m * n != G->group.order ) ) {
        qWarning( "Invalid m,n for Znm isomorphic link command: %u,%u (Z_%u)",
                  m, n, G->group.order );
        return;
    }
    ...
    if ( !( gm && gn ) ) {
        qWarning( "Znm isomorphism link unable to find elements of orders %u,%u in group %s",
                  m, n, G->getPrimaryName().text.toLatin1().constData() );
        return;
    }
    GEltList tmp( 2 );
    tmp[0] = gm;
...
void ShowNoZnmIsomorphismLC::redo ()
{
    ...
    uint n = parameters[2].toUInt();
    if ( ( m < 2 ) || ( n < 2 ) || ( m * n != G->group.order ) ) {
        qWarning( "Invalid m,n for Znm nonisomorphic link command: %u,%u (Z_%u)",
                  m, n, G->group.order );
        return;
    }
    ...
    if ( !P ) {
        qWarning( "Znm nonisomorphism could not find group like %s",
                  elt_list_to_string( table ).toLatin1().constData() );
        return;
    }
    ...
    qDebug( "Completing gens %s gives gens %s",
            elt_list_to_code( tmp ).toLatin1().constData(),
